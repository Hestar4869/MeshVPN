...
/**
 * A handle representing wintun_ adapter
 */
typedef struct _WINTUN_ADAPTER *WINTUN_ADAPTER_HANDLE;

/**
 * Creates a new wintun_ adapter.
 *
...
/**
 * Opens an existing wintun_ adapter.
 *
 * @param Name          The requested name of the adapter. Zero-terminated string of up to MAX_ADAPTER_NAME-1
 *                      characters.
 *
...
/**
 * Releases wintun_ adapter resources and, if adapter was created with WintunCreateAdapter, removes adapter.
 *
...
/**
 * Deletes the wintun_ driver if there are no more adapters in use.
 *
...
/**
 * Determines the version of the wintun_ driver currently loaded.
 *
 * @return If the function succeeds, the return value is the version number. If the function fails, the return value is
 *         zero. To get extended error information, call GetLastError. Possible errors include the following:
 *         ERROR_FILE_NOT_FOUND  wintun_ not loaded
 */
...
/**
 * A handle representing wintun_ session
 */
typedef struct _TUN_SESSION *WINTUN_SESSION_HANDLE;

/**
 * Starts wintun_ session.
 *
 ...
 *
 * @return wintun_ session handle. Must be released with WintunEndSession. If the function fails, the return value is
...
/**
 * Ends wintun_ session.
 *
 * @param Session       wintun_ session handle obtained with WintunStartSession
 */
typedef VOID(WINAPI WINTUN_END_SESSION_FUNC)(_In_ WINTUN_SESSION_HANDLE Session);

/**
 * Gets wintun_ session's read-wait event handle.
 *
 * @param Session       wintun_ session handle obtained with WintunStartSession
 *
 ...
 *         following:
 *         ERROR_HANDLE_EOF     wintun_ adapter is terminating;
 *         ERROR_NO_MORE_ITEMS  wintun_ buffer is exhausted;
 *         ERROR_INVALID_DATA   wintun_ buffer is corrupt
 */
typedef _Must_inspect_result_
_Return_type_success_(return != NULL)
_Post_maybenull_
 ...
 *         following:
 *         ERROR_HANDLE_EOF       wintun_ adapter is terminating;
 *         ERROR_BUFFER_OVERFLOW  wintun_ buffer is full;
 */
typedef _Must_inspect_result_
_Return_type_success_(return != NULL)
_Post_maybenull_
